 ...
 * @LastEditTime: 2020-10-14 11:14:33
 */

// 主颜色
const $mainColor = 'FF5B78'
// 弹窗图标url
const $iconUrl = './images/ic_ar.png'
...
class Pushy {
  ...
  setConfig(options) {
    // 公开可重写的设置key
    const _publicSettings = ['projectId', 'update', 'updateUrl', 'log']
  ...
  async onRequestUpdate() {
    return new Promise((resolve, reject) => {
      ...
      uni.request({
        ...
        success: res => {
          const data = res.data
          // this._config.log && (console.log('Uni - pushy>>>>>>>>>', data, this))
          ...
          if (data.success) {
            ...
            if (native && native.versionCode > nativeVersionCode) {
              ...
              this._config.log &&
                console.log('Uni pushy：暂无新版本，接口响应 >>>', data)
              this._config.log &&
                console.log('Uni pushy：暂无新版本，本地版本 >>>', this)
              resolve(false)
            }
          } else {
            this._config.log &&
              console.log('Uni - pushy：success: false >>>>>>>>>', data)
            resolve(false)
          }
        },
        fail: e => {
